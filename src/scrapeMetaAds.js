console.log("GRAPHQL_DEBUG_MARKER: scrapeMetaAds.js loaded");
  ...
  async function handleLoginWall(page, options) {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      console.log('LOGIN REQUIRED: Please log in to Facebook in the opened browser. Press ENTER in
  ...
  function extractPageId(url) {
    try {
      ...
      return parsed.searchParams.get('view_all_page_id') || parsed.searchParams.get('id') ||
  'unknown';
    } catch (err) {
      return 'unknown';
    }
  ...
  function extractAdsFromNode(node, adMap) {
    ...
    if (node.ad_archive_id || node.adArchiveID || node.archive_id) {
      ...
      const ad = {
        ...
        started_running_on: node.started_running_on || node.startedRunningOn || node.start_time ||
  null,
        primary_text: node.ad_creative_body || node.ad_creative_body_text || node.adCreativeBody ||
  node.body || node.text || '',
  ...
  async function scrapeMetaAds(competitor, options) {
    ...
    if (!finalUrlInput || shouldSkipCompetitorUrl(finalUrlInput)) {
      console.error('[competitor] BAD URL: ' + (rawInput || finalUrlInput) + ' (missing
  view_all_page_id or search_term)');
      return {
        ads: [],
    ...
    await context.addCookies([
      ...
      { name: 'wd', value: '1920x1080', domain: '.facebook.com', path: '/' }
    ]);
    ...
    try {
      ...
      async function printGraphqlDebugOnce(tag, response) {
        ...
        try {
          ...
          console.log(`GRAPHQL_DEBUG_MARKER: ${tag} failed_to_read=${String(e && e.message ?
  e.message : e)}`);
        }
      }
      ...
      function parseGraphqlText(raw) {
        ...
        try {
          ...
          if (!graphqlParseFailedLogged) {
            console.log('GRAPHQL_DEBUG_MARKER: json_parse_failed err=' + (err && err.message ?
  err.message : String(err)) + ' first80=' + cleaned.slice(0, 80));
      ...
      if ((finalUrlInput.includes('sort_data') || (!finalUrlInput.includes('view_all_page_id=') && !
  finalUrlInput.includes('search_term=')))) {
        console.error('[nav] BAD FINAL URL (generic or missing view_all_page_id). Refusing to
  scrape.');
      ...
      console.log(`[scrape] GraphQL responses seen: ${graphqlResponsesSeen}, parsed: ${graphqlParsed},
  ad ids: ${graphqlAdIds}`);
      console.log(`GRAPHQL_DEBUG_MARKER: summary responsesSeen=${graphqlResponsesSeen}
  parsed=${graphqlParsed} ids=${graphqlAdIds} domLinks=${domAdLinksFound}
  cookieAccepted=${cookieAccepted ? 'yes' : 'no'}`);
